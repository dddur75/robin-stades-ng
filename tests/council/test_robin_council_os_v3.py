...
def committed_artifact_sha256(repo: Path, revision: str, relative: str) -> str:
...
def test_jalon4_wall_clock_decay_fix_authority_and_succession_are_exact() -> None:
    ...
    assert len(graph["claims"]) >= 424
    assert len(graph["decision_nodes"]) >= 169
    assert len(graph["edges"]) >= 722
    expected_statuses = {
...
def test_real_execution_bootstrap_closure_v1_authority_and_succession_are_exact() -> None:
    ...
    graph = json.loads(
        subprocess.run(
            [
                "git",
                "show",
                "9398ca95142eea7672bb1cff795379bc45ba96fe:reports/evidence/evidence-graph.json",
            ],
            cwd=ROOT,
            check=True,
            capture_output=True,
    ...
    assert {claim_id: claims[claim_id]["status"] for claim_id in expected_proof} == {
        ...
        expected_proof[1]: "SUPERSEDED",
        expected_proof[2]: "SUPERSEDED",
        expected_proof[3]: "SUPERSEDED",
        **{claim_id: "VERIFIED" for claim_id in expected_proof[4:9]},
    ...
    assert {claim_id: claims[claim_id]["status"] for claim_id in accounting_correction_proof} == {
        accounting_correction_proof[0]: "SUPERSEDED",
        accounting_correction_proof[1]: "SUPERSEDED",
        accounting_correction_proof[2]: "VERIFIED",
        accounting_correction_proof[3]: "SUPERSEDED",
        **{claim_id: "VERIFIED" for claim_id in accounting_correction_proof[4:8]},
        accounting_correction_proof[8]: "SUPERSEDED",
    }
...
def test_pr67_atomic_evidence_closure_is_exact_append_only_and_effect_free() -> None:
    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.025",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.034",
        "TEMPORAL.OWNER_REVIEW_PACK.GENERATED_AT.CANONICALIZATION.V1.001",
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.001",
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.001",
        "GOV.CI.REAL_EXECUTION_BOOTSTRAP.CLOSURE.WORKFLOW.V1.002",
        "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.005",
    ]
    expected_objections = [
        "No historical ledger rewrite is permitted.",
        "No silent replacement of the old workflow claim is permitted.",
        "The four functional PR67 files must remain byte-identical.",
        "The full local suite must pass before commit.",
        "The next exact-head CI must complete before any Ready or merge decision.",
        "No fourth autonomous correction is authorized after the push.",
        "No live effect or real Owner Review Pack is authorized.",
        "Merge remains conditional on unchanged main and clean mergeability.",
    ]
    ledger_path = ROOT / "reports/council/decision-ledger.jsonl"
    ledger_bytes = ledger_path.read_bytes()
    assert ledger_bytes.endswith(b"\n")
    assert not ledger_bytes.startswith(b"\xef\xbb\xbf")
    assert b"\r\n" not in ledger_bytes
    ledger_lines = ledger_bytes.splitlines()
    assert all(ledger_lines)

    records = [json.loads(line) for line in ledger_lines]
    assert records[166]["decision_id"] == "RCV3-20260822-174"
    assert records[166]["hash"] == (
        "c34e27b363ab37f48f23b2b88d3de3f06dd0e2487a0315dd35e03212d8dd6a68"
    )
    record = records[167]
    assert record["decision_id"] == "RCV3-20260823-175"
    assert record["record_type"] == "DECISION"
    assert record["date"] == "2026-08-23T18:58:34Z"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    assert record["previous_hash"] == records[166]["hash"]
    assert record["proof"] == expected_proof
    assert record["objections"] == expected_objections
    ...
    for item in records:
        ...
        assert hashlib.sha256(canonical).hexdigest() == item["hash"]
        previous_hash = item["hash"]

    prior_ledger = subprocess.run(
        [
            "git",
            "show",
            "ccf9670a2734d80760160360589180806d9ec7d3:reports/council/decision-ledger.jsonl",
        ],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ).stdout
    assert len(prior_ledger.splitlines()) == 167
    pr67_ledger = b"\n".join(ledger_lines[:168]) + b"\n"
    assert pr67_ledger.startswith(prior_ledger)
    assert pr67_ledger[len(prior_ledger) :] == ledger_lines[167] + b"\n"
    ...
    assert context["phase"] == (
        "OWNER_REVIEW_PACK_PR67_ATOMIC_CLOSURE_PRECOMMIT_EVIDENCE_SUCCESSION"
    )
    assert context["branch"] == "fix/owner-review-pack-generated-utc-v1"
    assert context["base_revision"] == "ccf9670a2734d80760160360589180806d9ec7d3"
    assert context["candidate_parent_sha"] == "d77741a09d2eab8a593cdac85ef7235fc8b17345"
    assert context["pr"] == 67
    ...
    assert context["pull_request_files"] == [
    ...
    assert context["scope"] == {
        "closure_changed_paths": 3,
        "pull_request_changed_paths": 7,
        "frozen_existing_paths": 4,
        "outside_authority": [],
    }
    assert context["closure_manifest"] == {
        "name": "pr67-atomic-closure-manifest.json",
        "location": "OUTSIDE_REPOSITORY_TEMPORARY",
    ...
    assert context["final_artifact_hashes"] == {
        "algorithm": "CANONICAL_GIT_LF_SHA256",
        "builder_sha256": ("1e4028d049db3d88ccf90ef6079bc3ad7f1efa6e1417b89c9c70bbbdad188a19"),
        "successor_test_sha256": (
            "82239742b4be228b8b0e72e1cf9d52ca50d4c34dcedc7bcf81068513ad67c2e2"
        ),
        "workflow_sha256": ("9e42262917dc1fc84acebedaacefe79fb233231aa3ff91baf6f4fee829f8c61d"),
        "portability_test_sha256": (
            "4643c0e1639d289d8bd71543d14abfbc63a8a3b69fc64cc496b1d3b3cf07732f"
        ),
        "council_test_sha256": ("89e52a96195347e05e1f08d8cce6070624e829f0484a46bd66108ce45eabb54a"),
        "matrix_sha256": ("6777e609247356a9e93fb089a928d25f28c5ba7f9d3fb39a199130d6e866f5ef"),
        "previous_ledger_sha256": (
            "7f58c7bcfcdb2002ba2e760332a3421d6b33ab2561a2fad5c8c20c08cfe01962"
        ),
        "final_ledger_sha256_bound_by_claim": expected_proof[6],
    }

    diagnosed_ci = context["diagnosed_ci"]
    assert diagnosed_ci["run_32652510629"] == {
        "event": "pull_request",
        "attempt": 1,
        "head_sha": "b4b1311e1b405bfce7f7f36145898ca2fc9a9d9e",
        "status": "completed",
        "conclusion": "cancelled",
        "windows_job_id": 97226193916,
        "windows_pytest": "404 passed in 519.88s",
        "classification": "FUNCTIONAL_SUCCESS_TIMEOUT_CANCELLATION",
    }
    second_run = diagnosed_ci["run_32654888150"]
    assert second_run["event"] == "pull_request"
    assert second_run["attempt"] == 1
    assert second_run["head_sha"] == "d77741a09d2eab8a593cdac85ef7235fc8b17345"
    assert second_run["status"] == "completed"
    assert second_run["conclusion"] == "failure"
    assert second_run["windows_proof"] == {
        "job_id": 97231980852,
        "result": "success",
        "pytest": "404 passed in 410.60s",
    }
    assert second_run["common_root_cause"] == (
        "STALE_ACTIVE_EVIDENCE_CLAIM_AFTER_AUTHORIZED_WORKFLOW_CHANGE"
    )
    assert second_run["failed_job_surfaces"] == [
        {
            "job_id": 97233562427,
            "name": "Chronos PostgreSQL profiles / tests (superuser)",
            "classification": "COMMON_ROOT_CAUSE_SURFACE",
        },
        {
            "job_id": 97233562449,
    ...
    assert second_run["independent_root_cause_count"] == 1
    ...
    assert context["prewrite_tests"] == {
        "red_evidence_graph": "EXPECTED_FAILURE_CONFIRMED_F6F76A_VS_9E4226",
        "owner_pack_regression": "PASS",
        "windows_portability_contract": "PASS",
    }
    expected_test_commands = [
        (
            "targeted_evidence_graph",
            "python -m pytest -q -p no:cacheprovider "
            "tests/council/test_robin_council_os_v3.py::"
            "test_evidence_graph_and_append_only_ledger_have_mandatory_fields",
        ),
        (
            "new_atomic_closure_test",
            "python -m pytest -q -p no:cacheprovider "
            "tests/council/test_robin_council_os_v3.py::"
            "test_pr67_atomic_evidence_closure_is_exact_append_only_and_effect_free",
        ),
        (
            "full_council",
            "python -m pytest -q -p no:cacheprovider tests/council/test_robin_council_os_v3.py",
        ),
        (
        ...
        (
            "owner_pack_regression",
            "python -m pytest -q -p no:cacheprovider "
            "tests/capture/test_live_canary_successor_v2.py::"
            "test_owner_review_pack_cli_canonicalizes_generated_at_for_nonce",
        ),
        (
            "full_successor",
            "python -m pytest -q -p no:cacheprovider "
        ...
        (
            "full_capture",
            "python -m pytest -q -p no:cacheprovider tests/capture",
        ),
        (
            "workflow_contract",
            "python -m pytest -q -p no:cacheprovider "
    ...
    assert context["final_precommit_tests"] == [
        {
            "name": name,
            "command": command,
            "result": "PASS",
            "observation_gate": "MUST_BE_OBSERVED_BEFORE_COMMIT",
        }
        for name, command in expected_test_commands
    ]
    assert context["final_static_validations"] == [
        {"name": name, "result": "PASS", "observation_gate": "MUST_BE_OBSERVED_BEFORE_COMMIT"}
        for name in (
            "JSON_VALIDATION",
            "LEDGER_CHAIN",
            "ACTIVE_CLAIM_VALIDATOR",
            "RUFF_CHECK",
            "RUFF_FORMAT_CHECK",
            "COMPILEALL",
            "SECRET_SCAN",
    ...
    assert context["defects"] == {
        ...
        "open_p2": 0,
        "open_critical_threads": 0,
        "active_claim_hash_mismatches_before": 1,
        "active_claim_hash_mismatches_after": 0,
        "broken_successions_after": 0,
        "broken_ledger_links_after": 0,
    }
    assert context["external_effects"] == {
        "official_schedule_reads": 0,
        "campaign_selections": 0,
        "real_dns": 0,
        "provider_tcp": 0,
        "provider_http": 0,
        "real_secret_reads": 0,
        "real_owner_review_pack_builds": 0,
        "real_owner_authorizations": 0,
        "real_activations": 0,
        "real_captures": 0,
        "promotions": 0,
    ...
    assert context["delivery_authority"] == {
        "one_final_atomic_commit_authorized_now": True,
        "one_final_non_force_push_authorized_now": True,
        "automatic_exact_head_ci_required": True,
        "ready_authorized_only_after_exact_head_ci_green": True,
        "merge_commit_authorized_only_after_exact_head_ci_green": True,
        "postmerge_ci_required": True,
        "additional_corrective_commit_after_push": False,
        "additional_corrective_push_after_push": False,
        "force_push_authorized": False,
        "squash_merge_authorized": False,
        "rebase_merge_authorized": False,
        "admin_bypass_authorized": False,
    ...
    assert context["outcome_boundary"] == {
        "engineering_closure_candidate_complete": True,
        "fresh_owner_review_pack_mission_may_start_only_after_merge_and_postmerge_ci": True,
        "real_owner_review_pack_claimed": False,
        "owner_authorization_ready": False,
    }
    ...
    assert len(graph["decision_nodes"]) >= 169
    assert len(graph["edges"]) >= 722
    assert [claim["claim_id"] for claim in graph["claims"][409:416]] == (expected_proof)

    successions = {
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.024": expected_proof[0],
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.033": expected_proof[1],
        "GOV.CI.REAL_EXECUTION_BOOTSTRAP.CLOSURE.WORKFLOW.V1.001": expected_proof[5],
        "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.004": (
            expected_proof[6]
        ),
    }
    ...
    assert claims["GOV.CI.REAL_EXECUTION_BOOTSTRAP.CLOSURE.WORKFLOW.V1.001"]["hash"] == (
        "f6f76aa8ffa4631a18e8a650aa380959757bdd3d0ab02326fe79a58df865ec11"
    )
    assert claims["GOV.AUTHORIZATION.CHRONOS_LOOP53.024"]["hash"] == (
        "6777e609247356a9e93fb089a928d25f28c5ba7f9d3fb39a199130d6e866f5ef"
    )
    assert claims["GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.033"]["hash"] == (
        "0e1dd8fdfda9fc3a6121765c80722d3f21e0c2e65ef167d35bd67ac4d8cd1b8d"
    )
    assert (
        claims["GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.004"][
            "hash"
        ]
        == "7f58c7bcfcdb2002ba2e760332a3421d6b33ab2561a2fad5c8c20c08cfe01962"
    )
    ...
    assert {claim_id: claims[claim_id]["status"] for claim_id in expected_proof} == {
        expected_proof[0]: "SUPERSEDED",
        expected_proof[1]: "SUPERSEDED",
        expected_proof[2]: "VERIFIED",
        expected_proof[3]: "SUPERSEDED",
        expected_proof[4]: "SUPERSEDED",
        expected_proof[5]: "VERIFIED",
        expected_proof[6]: "SUPERSEDED",
    }
    assert all(
        claims[claim_id]["verified_by"] == ["C0", "C2", "C4", "DP6"] for claim_id in expected_proof
    )
    assert claims[expected_proof[2]]["hash"] == (
        "1e4028d049db3d88ccf90ef6079bc3ad7f1efa6e1417b89c9c70bbbdad188a19"
    )
    assert claims[expected_proof[3]]["hash"] == (
        "82239742b4be228b8b0e72e1cf9d52ca50d4c34dcedc7bcf81068513ad67c2e2"
    )
    assert claims[expected_proof[4]]["hash"] == claims[expected_proof[3]]["hash"]
    assert "without timespec" in claims[expected_proof[2]]["source"]
    assert "synthetic pack" in claims[expected_proof[4]]["source"]
    assert "static DP6 and C4 path review" in claims[expected_proof[4]]["source"]

    active_workflow_claims = [
        claim
        for claim in graph["claims"]
        if claim["artifact"] == ".github/workflows/ci.yml"
        and claim["status"] not in {"SUPERSEDED", "INVALIDATED"}
    ]
    assert [(claim["claim_id"], claim["hash"]) for claim in active_workflow_claims] == [
        (
            "GOV.CI.REAL_EXECUTION_BOOTSTRAP.CLOSURE.WORKFLOW.V1.002",
            "9e42262917dc1fc84acebedaacefe79fb233231aa3ff91baf6f4fee829f8c61d",
        )
    ]
    assert claims[expected_proof[1]]["hash"] == (
        "89e52a96195347e05e1f08d8cce6070624e829f0484a46bd66108ce45eabb54a"
    )
    assert claims[expected_proof[6]]["hash"] == (
        "9d97bee27d268dfda72fa45c09de5f3297680cf51fee0e09e8ec707245ea7f72"
    )

    frozen_blobs = {
        "tools/data-sourcing/build_owner_review_pack_v1.py": (
            "ab723b8c5d7e289e54815904531e7e432eb5c1a0"
        ),
        "tests/capture/test_live_canary_successor_v2.py": (
            "44040eba18de4b0a42f64b84ad003c822befb4c7"
        ),
        ".github/workflows/ci.yml": "b7b6f910c02eddebe52488b43914492a4075c3a1",
        "tests/portability/test_chronos_portable_ci_contract.py": (
            "6d5efa5a5472b12c58cbc16f2ad2123a58b808eb"
        ),
    }
    assert {
        path: _git(
            ROOT,
            "rev-parse",
            f"826dfc1e1cd4b1c1eb14608517c5e4fa5e6a072d:{path}",
        )
        for path in frozen_blobs
    } == frozen_blobs
    assert {
        path: hashlib.sha256(
            subprocess.run(
    ...
    expected_edges = [
        {
            ...
            "to_decision_id": "RCV3-20260823-175",
            "relation": "SUPPORTS",
            "status": "RECORDED",
        }
        for number, claim_id in zip(range(708, 715), expected_proof, strict=True)
    ]
    assert graph["edges"][706]["edge_id"] == "EDGE.707"
    assert graph["edges"][707:714] == expected_edges
    assert graph["decision_nodes"][167] == {
        "decision_id": "RCV3-20260823-175",
...
def test_strict_authority_rotation_v2_is_exact_append_only_and_effect_free() -> None:
    ...
    start_blobs = {
        "configs/execution/real-execution-bootstrap-closure-v1.json": (
            "a1dc865fe21866dd2b8ee889fbf37b9b1ce667b0"
        ),
        "src/robin/capture/bootstrap_contracts.py": ("3f44eaeb96d243d70da0827477be71c344a897fc"),
        "tests/capture/test_real_capture_workspace_bootstrap.py": (
            "653cf25f764c38c422585bb6f5564acb7ac71469"
        ),
        "tests/capture/test_live_canary_successor_v2.py": (
            "44040eba18de4b0a42f64b84ad003c822befb4c7"
        ),
        "tests/council/test_real_execution_bootstrap_governance.py": (
            "cc92b9a70d4c534b091665df8043d323ecc73bea"
        ),
        "reports/evidence/evidence-graph.json": ("7dcef3cd099e9f7378a8bd8edcce927df551a7df"),
        "reports/council/decision-ledger.jsonl": ("39bafce1d0fd447da0c3e3c7edf86518a796cb2a"),
        "tests/council/test_robin_council_os_v3.py": ("0ca05f929fe5929bf9fbef7232b5276a08cba215"),
    }
    assert strict_paths == list(start_blobs)
    assert {
    ...
    effects = json.dumps(manifest["external_effects"]).casefold()
    assert "provider_http" not in effects
    ...
    assert typed_contract.count(new_source_hash) == 3
    assert workspace_fixture.count(new_source_hash) == 1
    assert successor_fixture.count(new_source_hash) == 1
    assert f'SOURCE_HASH = "{new_source_hash}"' in bootstrap_governance
    assert f'OLD_SOURCE_HASH = "{old_source_hash}"' in bootstrap_governance

    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.026",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.035",
        "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.V1.002",
        ("GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.SOURCE_HASH.CONTRACT.V1.001"),
        (
            "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.WORKSPACE.FULL_CLONE."
            "JOB_ACCOUNTING.REGRESSION.V1.002"
        ),
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.002",
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.002",
        ("GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.006"),
    ]
    successors = {
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.025": expected_proof[0],
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.034": expected_proof[1],
        ("GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.V1.001"): expected_proof[2],
        (
            "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.WORKSPACE.FULL_CLONE."
            "JOB_ACCOUNTING.REGRESSION.V1.001"
        ): expected_proof[4],
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.001": expected_proof[5],
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.001": expected_proof[6],
        (
            "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.005"
        ): expected_proof[7],
    }

    ledger_path = ROOT / strict_paths[6]
    ledger_bytes = ledger_path.read_bytes()
    baseline_ledger = subprocess.run(
        ["git", "show", f"{start_main}:{strict_paths[6]}"],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ).stdout
    ledger_lines = ledger_bytes.splitlines()
    assert len(baseline_ledger.splitlines()) == 168
    assert len(ledger_lines) >= 169
    assert ledger_bytes.startswith(baseline_ledger)
    assert ledger_bytes[len(baseline_ledger) :].startswith(ledger_lines[168] + b"\n")
    assert b"\r" not in ledger_bytes

    records = [json.loads(line) for line in ledger_lines]
    record = records[168]
    assert record["decision_id"] == "RCV3-20260824-176"
    assert record["record_type"] == "DECISION"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    assert record["previous_hash"] == (
        "444c35a52c0f47693a3a0f796e3d28fda82e54e47f5e0f97d30143e634fc1077"
    )
    assert record["proof"] == expected_proof
    assert len(record["objections"]) == 12
    ...
    assert context["commit_context"] is False
    assert context["mission_id"] == "REAL_EXECUTION_BOOTSTRAP_CLOSURE_V1"
    assert context["phase"] == ("FRESH_OWNER_REVIEW_PACK_AUTHORITY_V2_STRICT_ROTATION_PRECOMMIT")
    assert context["base_revision"] == start_main
    assert context["candidate_parent_sha"] == start_main
    assert context["branch"] == "codex/fresh-owner-review-pack-authority-v2"
    assert context["pr"] == "DRAFT_PENDING"
    assert context["writer"] == "C0"
    assert context["files"] == strict_paths
    assert context["strict_rotation_start_blobs"] == start_blobs
    assert context["closure_manifest"] == {
        "name": "strict-authority-v2-8-file-closure-manifest.json",
        "location": "OUTSIDE_REPOSITORY_TEMPORARY",
        "closure_complete": True,
        "out_of_scope_dependencies": [],
    }
    assert context["authority_hashes"] == {
        "owner_directive_sha256": new_source_hash,
        "old_source_hash": old_source_hash,
        "new_source_hash": new_source_hash,
        "old_manifest_canonical_sha256": (
            "276e30fc4fe23600f654f93d9939b66b033d6002b68e86bcbfee0c34efe96c99"
        ),
        "new_manifest_canonical_sha256": (
            "5f8d1db2586adbc397d1b4e814e85946c84327537872df6aeded0fbbbc5498c0"
        ),
    }
    assert context["external_effects"] == {
        "official_schedule_logical_reads": 0,
        "corpus_snapshots": 0,
        "provider_dns": 0,
        "provider_tcp": 0,
        "provider_http": 0,
    ...
    assert context["conditional_authority"] == {
        "one_engineering_commit": True,
        "one_non_force_push": True,
        "one_draft_pr": True,
        "merge_commit_after_exact_head_ci": True,
        "postmerge_ci_required": True,
        "workspace_create_after_postmerge_ci": True,
        "workspace_verify_after_postmerge_ci": True,
        "official_schedule_logical_reads_maximum": 10,
        "corpus_snapshots_maximum": 2,
        "provider_public_dns_resolution_maximum": 1,
        "real_owner_review_pack_build_maximum": 1,
        "provider_tcp_connections": 0,
        "provider_http_calls": 0,
        "real_secret_reads": 0,
        "owner_authorized_artifacts": 0,
    ...
    assert context["defects"] == {
        "active_claim_hash_mismatches_before": 0,
        "active_claim_hash_mismatches_after": 0,
        "broken_successions_before": 0,
        "broken_successions_after": 0,
        "broken_ledger_links_before": 0,
        "broken_ledger_links_after": 0,
        "open_p0": 0,
        "open_p1": 0,
        "open_p2": 0,
    ...
    assert all(item["result"] == "PASS" for item in context["final_precommit_tests"])
    assert all(item["result"] == "PASS" for item in context["final_static_validations"])

    baseline_graph = json.loads(
        subprocess.run(
            ["git", "show", f"{start_main}:{strict_paths[5]}"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
    ...
    graph = json.loads(
        subprocess.run(
            ["git", "show", f"9398ca95142eea7672bb1cff795379bc45ba96fe:{strict_paths[5]}"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
    )
    assert len(baseline_graph["claims"]) == 416
    assert len(baseline_graph["decision_nodes"]) == 168
    assert len(baseline_graph["edges"]) == 714
    assert len(graph["claims"]) >= 424
    assert len(graph["decision_nodes"]) >= 169
    assert len(graph["edges"]) >= 722
    ...
    for claim in baseline_graph["claims"]:
        if claim["claim_id"] in successors:
            claim = {
                **claim,
                "status": "SUPERSEDED",
                "superseded_by": successors[claim["claim_id"]],
            }
        expected_historical_claims.append(claim)
    assert graph["claims"][:416] == expected_historical_claims
    assert [claim["claim_id"] for claim in graph["claims"][416:424]] == expected_proof
    later_successors = {
        expected_proof[1]: "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.036",
        expected_proof[3]: (
            "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST."
            "SOURCE_HASH.CONTRACT.V1.002"
        ),
        expected_proof[5]: "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.003",
        expected_proof[6]: "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.003",
        expected_proof[7]: (
            "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.007"
        ),
    }
    assert claims[expected_proof[0]]["status"] == "PARTIAL"
    assert claims[expected_proof[2]]["status"] == "VERIFIED"
    assert claims[expected_proof[4]]["status"] == "VERIFIED"
    for predecessor, successor in later_successors.items():
        assert claims[predecessor]["status"] == "SUPERSEDED"
        assert claims[predecessor]["superseded_by"] == successor
    for predecessor, successor in successors.items():
        assert claims[predecessor]["status"] == "SUPERSEDED"
        assert claims[predecessor]["superseded_by"] == successor
        assert claims[successor]["successor_of"] == predecessor
    assert "successor_of" not in claims[expected_proof[3]]

    claim_artifacts = {
        expected_proof[0]: "configs/agents/mission-activation-matrix-v3.json",
        expected_proof[1]: strict_paths[7],
        expected_proof[2]: strict_paths[0],
        expected_proof[3]: strict_paths[1],
        expected_proof[4]: strict_paths[2],
        expected_proof[5]: strict_paths[3],
        expected_proof[6]: strict_paths[3],
        expected_proof[7]: strict_paths[6],
    }
    ...
    assert graph["decision_nodes"][168] == {
        "decision_id": record["decision_id"],
        "ledger_record_hash": record["hash"],
    }
    assert graph["edges"][:714] == baseline_graph["edges"]
    assert graph["edges"][714:722] == [
        {
            "edge_id": f"EDGE.{number}",
            "from_claim_id": claim_id,
        ...
        for number, claim_id in zip(range(715, 723), expected_proof, strict=True)
    ]


def test_first_c0_hardening_incident_closure_v2_is_append_only_and_effect_bounded() -> None:
    start_main = "2620f56c030c28e31128681d19f712f321079482"
    final_paths = [
        "pyproject.toml",
        "requirements.txt",
        "src/robin/capture/bootstrap_contracts.py",
        ...
        "tools/data-sourcing/prepare_owner_review_pack_inputs_v1.py",
        "tools/data-sourcing/run_owner_review_pack_once_v1.py",
        "tests/capture/conftest.py",
        "tests/capture/test_live_canary_successor_v2.py",
        "tests/capture/test_provider_network_binding.py",
        "tests/capture/test_official_schedule_sources_v1.py",
        "tests/capture/test_predns_orchestration_v1.py",
        "reports/council/first-c0-hardening-dns-incident-v1.json",
        "reports/evidence/evidence-graph.json",
        "reports/council/decision-ledger.jsonl",
        "tests/council/test_robin_council_os_v3.py",
    ]
    start_blobs = {
        "pyproject.toml": "8c06f2720b14c80e955182aedab5dc959b312a8a",
        "requirements.txt": "b43b39b8e3ba3eaf5d5c64927ce2c438e3b38b13",
        "src/robin/capture/bootstrap_contracts.py": ("337de33be67f7f02e1051e6f7cae1a442c369143"),
        "tests/capture/conftest.py": "ac102334498d63897c9a767c3fb2c7bb46a8ced1",
        "tests/capture/test_live_canary_successor_v2.py": (
            "9acea3e030df9acd1e202b6259bd22d80249194f"
        ),
        "tests/capture/test_provider_network_binding.py": (
            "775446f97eda83757ccfde124d4ec2a46f28eb94"
        ),
        "reports/evidence/evidence-graph.json": ("1a49ca42015cf1e6f2c09fa5c02908528453ed82"),
        "reports/council/decision-ledger.jsonl": ("b783f96c48a56663d1e3490bb5319ff6537dc387"),
        "tests/council/test_robin_council_os_v3.py": ("d95a7d5081d017fdc698e15cb0d070a32f2cf0c2"),
    }
    assert len(final_paths) == 16
    assert len(final_paths) <= 18
    assert {
        path: _git(ROOT, "rev-parse", f"{start_main}:{path}") for path in start_blobs
    } == start_blobs

    incident_closure_head = "9398ca95142eea7672bb1cff795379bc45ba96fe"
    observed_paths = committed_changed_paths(ROOT, start_main, incident_closure_head)
    assert observed_paths == set(final_paths)

    owner_directive = (
        "ROBIN_OWNER_DIRECTIVE_FIRST_C0_HARDENING_INCIDENT_CLOSURE_V2|2026-08-24|"
        "main=2620f56c030c28e31128681d19f712f321079482|"
        "acknowledged_engineering_dns_invocations=1|"
        "production_v2_marker_must_remain_absent|"
        "purpose=salvage_hardening_and_prevent_test_network_escape|tracked_files_max=18|"
        "engineering_commits_max=3|non_force_pushes_max=2|draft_prs=1|"
        "merge_commit_after_ci=1|postmerge_official_schedule_reads_max=20|"
        "postmerge_predns_rehearsal=1|future_provider_dns=0|provider_tcp=0|"
        "provider_http=0|real_secret_reads=0|owner_review_pack_builds=0|"
        "owner_authorization=not_created|activation=0|real_captures=0|promotion=0|bet=0"
    ).encode()
    assert b"\r" not in owner_directive
    assert not owner_directive.endswith(b"\n")
    assert hashlib.sha256(owner_directive).hexdigest() == (
        "4035c0019cdc16dee43ca68024fae1c4683b6774dff3f5f04d0a95f3d917b8fd"
    )

    incident = load_json("reports/council/first-c0-hardening-dns-incident-v1.json")
    assert incident["incident_id"] == "FIRST-C0-HARDENING-TEST-DNS-001"
    assert incident["exact_test"] == (
        "tests/capture/test_predns_orchestration_v1.py::"
        "test_real_runner_cli_defaults_to_preflight_and_requires_explicit_execute_owner_gate"
    )
    assert incident["hostname"] == "api.the-odds-api.com"
    assert incident["exact_call_path"] == [
        "test_real_runner_cli_defaults_to_preflight_and_requires_explicit_execute_owner_gate",
        "tools/data-sourcing/run_owner_review_pack_once_v1.py::main",
        "src/robin/capture/predns_orchestration.py::run_owner_review_pack_once_v1",
        "tests/capture/test_predns_orchestration_v1.py::fake_binding_preparer",
        "src/robin/capture/predns_orchestration.py::counted_resolver",
        "tools/data-sourcing/run_owner_review_pack_once_v1.py::_system_resolver",
        "socket.getaddrinfo",
    ]
    assert incident["resolver_operations"] == 1
    assert incident["acknowledged_engineering_dns_invocations"] == 1
    assert incident["technically_proven_resolver_wrapper_attempts"] == 1
    assert "DOES NOT INDEPENDENTLY PROVE" in incident["os_dns_lookup_evidence_status"]
    assert incident["provider_tcp"] == 0
    assert incident["provider_http"] == 0
    assert incident["secret_reads"] == 0
    assert incident["pack_builds"] == 0
    assert incident["authorization_artifacts"] == 0
    assert incident["capture_calls"] == 0
    assert incident["production_marker_status"] == {
        "v2_global_marker_present": False,
        "v2_local_mirror_present": False,
        "historical_marker_present": True,
        "historical_marker_unchanged": True,
    }
    assert incident["production_binding_status"]["v2_binding_present"] is False
    assert incident["production_v2_authority_intact_proven"] is True
    assert incident["production_v2_dns_budget_available"] is True
    assert incident["future_test_or_provider_dns_during_closure"] == 0
    assert incident["evidence_hashes"]["synthetic_hard_stop_receipt_sha256"] == (
        "516fa7cbeee34e9cc1027780dc2d208c641a34c922d7b994189c1ee8212a0444"
    )

    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.027",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.036",
        ("GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.SOURCE_HASH.CONTRACT.V1.002"),
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.003",
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.003",
        "SECURITY.FIRST_C0.HARDENING.TEST_DNS.INCIDENT.V1.001",
        "TEMPORAL.FIRST_C0.SELECTOR.ROLLOVER.V1.001",
        "SECURITY.FIRST_C0.TEST_NETWORK.DEFAULT_DENY.V1.001",
        "DATA.FIRST_C0.OFFICIAL_SCHEDULE.ADAPTERS.V1.001",
        "GOV.FIRST_C0.PRE_DNS.ORCHESTRATION.V1.001",
        "SECURITY.FIRST_C0.ATOMIC_DNS_TO_PACK.RUNNER.V1.001",
        ("GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.007"),
    ]
    successors = {
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.035": expected_proof[1],
        (
            "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST."
            "SOURCE_HASH.CONTRACT.V1.001"
        ): expected_proof[2],
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.002": expected_proof[3],
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.002": expected_proof[4],
        (
            "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.006"
        ): expected_proof[11],
    }
    ...
    ledger_bytes = ledger_path.read_bytes()
    baseline_ledger = subprocess.run(
        ["git", "show", f"{start_main}:reports/council/decision-ledger.jsonl"],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ).stdout
    ledger_lines = ledger_bytes.splitlines()
    assert len(baseline_ledger.splitlines()) == 169
    assert len(ledger_lines) >= 170
    assert ledger_bytes.startswith(baseline_ledger)
    assert ledger_bytes[len(baseline_ledger) :].startswith(ledger_lines[169] + b"\n")
    assert b"\r" not in ledger_bytes
    records = [json.loads(line) for line in ledger_lines]
    record = records[169]
    assert record["decision_id"] == "RCV3-20260824-177"
    assert record["record_type"] == "DECISION"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    assert record["previous_hash"] == (
        "c497d323ebe617c14a07e8b4c3a65d98f8adfbdedd2f5bfccbe6e093c7514bc6"
    )
    assert record["proof"] == expected_proof
    canonical_record = json.dumps(
        {key: value for key, value in record.items() if key != "hash"},
    ...
    mission_commits = _git(
        ROOT, "rev-list", "--reverse", "--no-merges", f"{start_main}..{incident_closure_head}"
    ).splitlines()
    ...
    assert context["pr"] == "DRAFT_PENDING"
    assert context["writer"] == "C0"
    assert context["files"] == final_paths
    assert context["start_blobs"] == start_blobs
    assert context["scope"] == {
        "tracked_files_maximum": 18,
        "changed_paths": 16,
        "direct_indispensable_extensions": ["pyproject.toml", "requirements.txt"],
        "outside_authorized_scope": [],
        "out_of_scope_dependencies": [],
    }
    assert context["closure_manifest"] == {
        "name": "first-c0-hardening-incident-closure-manifest-v2.json",
        "location": "OUTSIDE_REPOSITORY_TEMPORARY",
        "sha256": "c750233a14b6702b9ca43c8d6ccd82c245ac2c3cc6343ac2c0f3923cf6effb0b",
        "closure_complete": True,
        "tracked_path_count": 16,
        "out_of_scope_dependencies": [],
    }
    assert context["incident"]["acknowledged_engineering_dns_invocations"] == 1
    assert context["incident"]["resolver_operations"] == 1
    assert context["incident"]["provider_tcp"] == 0
    assert context["incident"]["provider_http"] == 0
    assert context["incident"]["real_secret_reads"] == 0
    assert context["incident"]["real_owner_review_pack_builds"] == 0
    assert context["production_authority"]["v2_global_marker_present"] is False
    assert context["production_authority"]["v2_local_mirror_present"] is False
    assert context["production_authority"]["v2_binding_present"] is False
    assert context["production_authority"]["dns_budget_available"] is True
    assert context["production_authority"]["manifest_source_hash"] == (
        "2451cd643c2d3ffcd3c5cc9fcd4a5f81f785978e0aa20429b4d182ceb9b1f22b"
    )
    assert context["dependency_boundary"] == {
        "pypdf_requirement": "pypdf>=6,<7",
        "pyproject_declared": True,
        "requirements_declared": True,
        "new_schema_version": False,
    }
    assert context["external_effects"]["acknowledged_prior_engineering_dns"] == 1
    assert context["external_effects"]["future_provider_dns"] == 0
    assert context["external_effects"]["provider_tcp"] == 0
    assert context["external_effects"]["provider_http"] == 0
    assert context["external_effects"]["real_secret_reads"] == 0
    assert context["external_effects"]["real_owner_review_pack_builds"] == 0
    assert context["conditional_authority"]["runner_mode_during_mission"] == ("PREFLIGHT_ONLY")
    assert context["conditional_authority"]["next_provider_dns_requires_david"] is True
    assert set(context["reviews"]) == {"DP6", "C4", "C2", "A2"}
    assert all(review["verdict"] == "ACCEPT" for review in context["reviews"].values())
    assert all(review["open_threads"] == 0 for review in context["reviews"].values())
    assert context["defects"] == {
        "active_claim_hash_mismatches_before": 3,
        "active_claim_hash_mismatches_after": 0,
        "broken_ledger_links_before": 0,
        "broken_ledger_links_after": 0,
        "broken_successions_before": 0,
    ...
    baseline_graph = json.loads(
        subprocess.run(
            ["git", "show", f"{start_main}:reports/evidence/evidence-graph.json"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
    ...
    graph = json.loads(
        subprocess.run(
            [
                "git",
                "show",
                f"{incident_closure_head}:reports/evidence/evidence-graph.json",
            ],
            cwd=ROOT,
            check=True,
            capture_output=True,
    ...
    assert len(baseline_graph["claims"]) == 424
    assert len(baseline_graph["decision_nodes"]) == 169
    assert len(baseline_graph["edges"]) == 722
    assert len(graph["claims"]) == 436
    assert len(graph["decision_nodes"]) == 170
    assert len(graph["edges"]) == 734
    assert len(current_graph["claims"]) >= 436
    assert len(current_graph["decision_nodes"]) >= 170
    assert len(current_graph["edges"]) >= 734
    claims = {claim["claim_id"]: claim for claim in graph["claims"]}
    baseline_claims = {claim["claim_id"]: claim for claim in baseline_graph["claims"]}
    expected_historical_claims = []
    for claim in baseline_graph["claims"]:
        if claim["claim_id"] in successors:
            claim = {
        ...
        expected_historical_claims.append(claim)
    assert graph["claims"][:424] == expected_historical_claims
    assert (
        claims["GOV.AUTHORIZATION.CHRONOS_LOOP53.026"]
        == baseline_claims["GOV.AUTHORIZATION.CHRONOS_LOOP53.026"]
    )
    assert [claim["claim_id"] for claim in graph["claims"][424:436]] == expected_proof
    assert claims[expected_proof[0]]["status"] == "PARTIAL"
    ...
    for predecessor, successor in successors.items():
        ...
        assert claims[predecessor]["superseded_by"] == successor
        assert claims[successor]["successor_of"] == predecessor
    assert "successor_of" not in claims[expected_proof[0]]
    assert "successor_of" not in claims[expected_proof[5]]

    claim_artifacts = {
        expected_proof[0]: "reports/council/first-c0-hardening-dns-incident-v1.json",
        expected_proof[1]: "tests/council/test_robin_council_os_v3.py",
        expected_proof[2]: "src/robin/capture/bootstrap_contracts.py",
        expected_proof[3]: "tests/capture/test_live_canary_successor_v2.py",
        expected_proof[4]: "tests/capture/test_live_canary_successor_v2.py",
        expected_proof[5]: "reports/council/first-c0-hardening-dns-incident-v1.json",
        expected_proof[6]: "src/robin/capture/bootstrap_contracts.py",
        expected_proof[7]: "tests/capture/conftest.py",
        expected_proof[8]: "src/robin/capture/official_schedule_sources.py",
        expected_proof[9]: "src/robin/capture/predns_orchestration.py",
        expected_proof[10]: "tools/data-sourcing/run_owner_review_pack_once_v1.py",
    ...
    for claim_id, artifact in claim_artifacts.items():
        ...
        assert claims[claim_id]["hash"] == committed_artifact_sha256(
            ROOT,
            incident_closure_head,
            artifact,
        )
        assert claims[claim_id]["verified_by"] == ["C0", "C2", "C4", "DP6", "A2"]

    assert graph["decision_nodes"][:169] == baseline_graph["decision_nodes"]
    assert graph["decision_nodes"][169] == {
        "decision_id": record["decision_id"],
        "ledger_record_hash": record["hash"],
    }
    assert graph["edges"][:722] == baseline_graph["edges"]
    assert graph["edges"][722:734] == [
        {
            "edge_id": f"EDGE.{number}",
            "from_claim_id": claim_id,
            "to_decision_id": record["decision_id"],
        ...
        for number, claim_id in zip(range(723, 735), expected_proof, strict=True)
    ]
    assert [claim["claim_id"] for claim in current_graph["claims"][424:436]] == expected_proof
    assert current_graph["decision_nodes"][169] == graph["decision_nodes"][169]
    assert current_graph["edges"][722:734] == graph["edges"][722:734]
    assert '"pypdf>=6,<7"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert (ROOT / "requirements.txt").read_text(encoding="utf-8").splitlines().count(
        "pypdf>=6,<7"
    ) == 1
    serialized = json.dumps({"record": record, "incident": incident}, ensure_ascii=False)
    assert "C:\\Users\\" not in serialized
    assert "zero DNS for the failed mission" not in serialized


def test_pr69_ci_artifact_scope_isolation_corrective_is_exact_and_append_only() -> None:
    start_main = "2620f56c030c28e31128681d19f712f321079482"
    candidate_parent = "9398ca95142eea7672bb1cff795379bc45ba96fe"
    closure_head = "f839b6ef711452512ad86bfd171b388cdf8471f7"
    corrective_paths = [
        "reports/council/decision-ledger.jsonl",
        "reports/evidence/evidence-graph.json",
        "tests/council/test_robin_council_os_v3.py",
    ]
    frozen_functional_blobs = {
        "pyproject.toml": "81e36ca3c644e1294bb2b74cff1edb96ffda2272",
        "requirements.txt": "fb75c5fa810479659a9b86e63e135f860bb9cbcf",
        "src/robin/capture/bootstrap_contracts.py": ("016a531756607474cecf3b52bc522fd81460dc2f"),
        "src/robin/capture/official_schedule_sources.py": (
            "664605c3ef3a6ad77034491a6200737f25657395"
        ),
        "src/robin/capture/predns_orchestration.py": ("cb3fe33650c9d840e467df4e73e8986bae916ba4"),
        "tools/data-sourcing/prepare_owner_review_pack_inputs_v1.py": (
            "60bf594ed8032f1c0bd3ef466565acaca1d36b98"
        ),
        "tools/data-sourcing/run_owner_review_pack_once_v1.py": (
            "d0c220fd9e59ef4d034bd69ded87cfe661400889"
        ),
        "tests/capture/conftest.py": "20747308ac9fd2220e3926fe7456558a49544ac8",
        "tests/capture/test_live_canary_successor_v2.py": (
            "ad8353f2e8f55bf358e0516988eff55bc8f44d6a"
        ),
        "tests/capture/test_provider_network_binding.py": (
            "a4ff0d81d474c09380e80cd6d4a5f6561027252f"
        ),
        "tests/capture/test_official_schedule_sources_v1.py": (
            "d545b5117511aefc75027b08b627940b8eae1fd4"
        ),
        "tests/capture/test_predns_orchestration_v1.py": (
            "ada0aeb2938d3ddef1d758db07d641c1af5fbf35"
        ),
        "reports/council/first-c0-hardening-dns-incident-v1.json": (
            "c9c3945f14d041b9754fa7913c499f4b747ba5ee"
        ),
    }
    assert len(corrective_paths) == 3
    assert len(frozen_functional_blobs) == 13
    for frozen_revision in (candidate_parent, closure_head):
        assert {
            path: _git(ROOT, "rev-parse", f"{frozen_revision}:{path}")
            for path in frozen_functional_blobs
        } == frozen_functional_blobs

    owner_directive = (
        "ROBIN_OWNER_DIRECTIVE_PR69_CI_ARTIFACT_SCOPE_ISOLATION_V1|2026-08-25|"
        "main=2620f56c030c28e31128681d19f712f321079482|pr=69|"
        "head=9398ca95142eea7672bb1cff795379bc45ba96fe|failed_run=32791282056|"
        "purpose=separate_committed_pr_scope_from_ci_worktree_artifacts|tracked_files=3|"
        "additional_commits=1|non_force_pushes=1|new_prs=0|merge_commit_after_ci=1|"
        "provider_dns=0|provider_tcp=0|provider_http=0|real_secret_reads=0|"
        "owner_review_pack_builds=0|owner_authorization=not_created|activation=0|"
        "real_captures=0|promotion=0|bet=0"
    ).encode()
    assert b"\r" not in owner_directive
    assert b"\n" not in owner_directive
    assert hashlib.sha256(owner_directive).hexdigest() == (
        "4cd9e9065e4876962b99c23a2d3a363b681b411773bbfa5eded3de510f6c94ff"
    )

    current_ledger_bytes = (ROOT / "reports/council/decision-ledger.jsonl").read_bytes()
    ledger_bytes = subprocess.run(
        ["git", "show", f"{closure_head}:reports/council/decision-ledger.jsonl"],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ).stdout
    parent_ledger = subprocess.run(
        ["git", "show", f"{candidate_parent}:reports/council/decision-ledger.jsonl"],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ).stdout
    assert len(parent_ledger.splitlines()) == 170
    assert len(ledger_bytes.splitlines()) == 171
    assert ledger_bytes.startswith(parent_ledger)
    assert ledger_bytes[len(parent_ledger) :] == ledger_bytes.splitlines()[-1] + b"\n"
    assert current_ledger_bytes.startswith(ledger_bytes)
    assert b"\r" not in ledger_bytes

    records = [json.loads(line) for line in ledger_bytes.splitlines()]
    previous_record = records[169]
    record = records[170]
    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.028",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.037",
        "GOV.FIRST_C0.PR69.CI.WORKTREE_SCOPE.ISOLATION.V1.001",
        ("GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.008"),
    ]
    assert previous_record["decision_id"] == "RCV3-20260824-177"
    assert record["decision_id"] == "RCV3-20260825-178"
    assert record["record_type"] == "DECISION"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    ...
    assert context["candidate_context"] is True
    assert context["commit_context"] is False
    assert context["mission_id"] == "PR69_CI_ARTIFACT_SCOPE_ISOLATION_V1"
    assert context["phase"] == "PR69_FINAL_DIRECTLY_CONSEQUENTIAL_CORRECTIVE_PRECOMMIT"
    assert context["base_revision"] == start_main
    assert context["candidate_parent_sha"] == candidate_parent
    assert context["branch"] == "codex/first-c0-hardening-incident-closure-v2"
    assert context["pr"] == 69
    assert context["writer"] == "C0"
    assert context["corrective_scope"] == {
        "changed_files": corrective_paths,
        "tracked_file_count": 3,
        "pr_total_changed_files": 16,
        "functional_code_changed": False,
        "workflow_changed": False,
        "dependencies_changed": False,
        "ambient_ci_artifact_whitelisted": False,
        "ambient_ci_artifact_cleaned": False,
    }
    assert context["frozen_functional_blobs"] == frozen_functional_blobs
    assert context["failed_exact_head_run"] == {
        "run_id": 32791282056,
        "job_id": 97639413227,
        "head_sha": candidate_parent,
        "status": "completed",
        "conclusion": "failure",
        "failing_test": (
            "tests/council/test_robin_council_os_v3.py::"
            "test_first_c0_hardening_incident_closure_v2_is_append_only_and_effect_bounded"
        ),
        "ambient_modified_path": "reports/pattern-research/campaign-summary.json",
        "root_cause_classification": (
            "COUNCIL_SCOPE_ASSERTION_CONFLATES_COMMITTED_PR_DIFF_WITH_CI_WORKTREE_STATE"
        ),
    }
    ...
    assert evidence["red"] == {
        ...
        "expected_paths": 16,
        "observed_paths": 17,
        "extra_path": "reports/pattern-research/campaign-summary.json",
        "semantic_json_equivalent": True,
    }
    assert evidence["green"] == {
        "result": "PASS",
        "exit_code": 0,
        "committed_scope_count": 16,
        "ambient_modified_path": "reports/pattern-research/campaign-summary.json",
        "ambient_modified_path_expands_scope": False,
    }
    assert evidence["negative_committed_expansion"] == {
        "result": "FAIL_EXPECTED",
        "exit_code": 1,
        "classification": "REAL_COMMITTED_SCOPE_EXPANSION_REJECTED",
        "extra_path": "pr69-ci-scope-negative-17th.txt",
    }
    assert all(item["result"] == "PASS" for item in context["local_tests"])
    assert all(item["result"] == "PASS" for item in context["static_validations"])
    assert set(context["reviews"]) == {"C2", "C4", "DP6", "A2"}
    assert all(review["verdict"] == "ACCEPT" for review in context["reviews"].values())
    assert all(review["open_threads"] == 0 for review in context["reviews"].values())
    assert context["defects"] == {
    ...
    assert all(value == 0 for value in context["external_effects"].values())
    assert context["delivery_authority"] == {
        "additional_commits": 1,
        "additional_non_force_pushes": 1,
        "new_pull_requests": 0,
        "conditional_merge_commits": 1,
        "merge_authorized_now": False,
        "exact_head_ci_required": True,
        "postmerge_ci_required": True,
        "postmerge_predns_only_after_green": True,
        "force_push_authorized": False,
        "branch_deletion_authorized": False,
        "provider_dns_authorized": False,
        "owner_review_pack_authorized": False,
    }

    parent_graph = json.loads(
        subprocess.run(
            ["git", "show", f"{candidate_parent}:reports/evidence/evidence-graph.json"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
    ...
    graph = json.loads(
        subprocess.run(
            ["git", "show", f"{closure_head}:reports/evidence/evidence-graph.json"],
            cwd=ROOT,
            check=True,
            capture_output=True,
        ).stdout
    ...
    current_graph = load_json("reports/evidence/evidence-graph.json")
    assert len(parent_graph["claims"]) == 436
    assert len(parent_graph["decision_nodes"]) == 170
    assert len(parent_graph["edges"]) == 734
    assert len(graph["claims"]) == 440
    assert len(graph["decision_nodes"]) == 171
    assert len(graph["edges"]) == 738
    assert len(current_graph["claims"]) >= 440
    assert len(current_graph["decision_nodes"]) >= 171
    assert len(current_graph["edges"]) >= 738

    successions = {
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.027": expected_proof[0],
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.036": expected_proof[1],
        (
            "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.007"
        ): expected_proof[3],
    }
    expected_parent_claims = []
    for claim in parent_graph["claims"]:
        if claim["claim_id"] in successions:
            claim = {
                **claim,
                "status": "SUPERSEDED",
                "superseded_by": successions[claim["claim_id"]],
            }
        expected_parent_claims.append(claim)
    assert graph["claims"][:436] == expected_parent_claims
    assert [claim["claim_id"] for claim in graph["claims"][436:]] == expected_proof
    ...
    for predecessor, successor in successions.items():
        ...
        assert claims[predecessor]["superseded_by"] == successor
        assert claims[successor]["successor_of"] == predecessor
    assert "successor_of" not in claims[expected_proof[2]]
    assert claims[expected_proof[0]]["status"] == "PARTIAL"
    assert claims[expected_proof[1]]["status"] == "PARTIAL"
    assert claims[expected_proof[2]]["status"] == "VERIFIED"
    assert claims[expected_proof[3]]["status"] == "VERIFIED"
    claim_artifacts = {
        expected_proof[0]: "reports/council/first-c0-hardening-dns-incident-v1.json",
        expected_proof[1]: "tests/council/test_robin_council_os_v3.py",
        expected_proof[2]: "tests/council/test_robin_council_os_v3.py",
        expected_proof[3]: "reports/council/decision-ledger.jsonl",
    }
    for claim_id, artifact in claim_artifacts.items():
        assert claims[claim_id]["artifact"] == artifact
        assert claims[claim_id]["hash"] == committed_artifact_sha256(
    ...
    assert graph["decision_nodes"][:170] == parent_graph["decision_nodes"]
    assert graph["decision_nodes"][170] == {
        "decision_id": record["decision_id"],
        "ledger_record_hash": record["hash"],
    }
    assert graph["edges"][:734] == parent_graph["edges"]
    assert graph["edges"][734:738] == [
        {
            "edge_id": f"EDGE.{number}",
            "from_claim_id": claim_id,
            "to_decision_id": record["decision_id"],
        ...
        for number, claim_id in zip(range(735, 739), expected_proof, strict=True)
    ]
    assert [edge["edge_id"] for edge in graph["edges"][:738]] == [
        f"EDGE.{number:03d}" for number in range(1, 739)
    ]
    assert [claim["claim_id"] for claim in current_graph["claims"][436:440]] == expected_proof
    assert current_graph["decision_nodes"][170] == graph["decision_nodes"][170]
    assert current_graph["edges"][734:738] == graph["edges"][734:738]


def test_first_real_c0_single_league_canary_is_exact_append_only_and_effect_free() -> None:
    base_revision = "bf98441751f4892023e15c4e5559520841df3bc8"
    changed_files = [
        "src/robin/capture/bootstrap_contracts.py",
        "src/robin/capture/provider_network.py",
    ...
    owner_directive = (
        "ROBIN_OWNER_DIRECTIVE_AUTONOMOUS_SINGLE_LEAGUE_C0_CLOSED_LOOP_V1|2026-08-25|"
        "main=bf98441751f4892023e15c4e5559520841df3bc8|"
        "continue_existing_diff=true|"
        "purpose=deliver_real_owner_review_pack_for_first_c0_canary_without_intermediate_escalation|"
        "tracked_files_max=16|local_fix_loops=unbounded_within_wall_clock|"
        "ci_corrective_cycles_max=3|non_force_pushes_max=4|pull_requests=1|"
        "official_physical_reads_max=12|preparation_cycles_max=3|"
        "primary_sport=soccer_spain_la_liga|"
        "fallback_sport=soccer_germany_bundesliga|"
        "provider_dns_resolutions_max=1|provider_tcp=0|provider_http=0|"
        "real_secret_reads=0|owner_review_pack_builds_max=1|"
        "owner_authorization=not_created|activation=0|real_captures=0|"
        "promotion=0|bet=0"
    ).encode()
    assert len(owner_directive) == 695
    assert b"\r" not in owner_directive
    assert b"\n" not in owner_directive
    assert hashlib.sha256(owner_directive).hexdigest() == (
        "0cd4bfe98162cb5bd4ec611b69fdff3a53fdc7bdc899ed856f3d5d38dacb067b"
    )
    ...
    if head_revision == base_revision:
        ...
        ledger_bytes = (ROOT / "reports/council/decision-ledger.jsonl").read_bytes()
    else:
        ...
        record_token = b'"decision_id":"RCV3-20260825-179"'
        eligible_closure_revisions = []
        for revision in candidate_revisions[:4]:
            ...
            if record_token not in candidate_ledger:
                continue
            if len(candidate_ledger.splitlines()) != 172:
                break
        ...
        if not eligible_closure_revisions:
            raise AssertionError("FIRST_C0_CLOSURE_COMMIT_NOT_FOUND")
        closure_revision, ledger_bytes = eligible_closure_revisions[-1]
        closure_commit_count = int(
            _git(
                ROOT,
                "rev-list",
                "--count",
        ...
        assert 1 <= closure_commit_count <= 4
    ...
    assert len(base_ledger.splitlines()) == 171
    assert len(ledger_bytes.splitlines()) == 172
    ...
    records = [json.loads(line) for line in ledger_bytes.splitlines()]
    previous_record = records[170]
    record = records[171]
    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.029",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.038",
        ("GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.SOURCE_HASH.CONTRACT.V1.003"),
        "TEMPORAL.FIRST_C0.SELECTOR.ROLLOVER.V1.002",
        "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.NETWORK.WINDOWS_API_TYPING.V1.002",
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.004",
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.004",
        "TEMPORAL.OWNER_REVIEW_PACK.GENERATED_AT.CANONICALIZATION.V1.002",
        "GOV.FIRST_C0.PR69.CI.WORKTREE_SCOPE.ISOLATION.V1.002",
        "GOV.FIRST_C0.PRE_DNS.ORCHESTRATION.V1.002",
        ("GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.009"),
    ]
    assert previous_record["decision_id"] == "RCV3-20260825-178"
    assert previous_record["hash"] == (
        "540ea8d6e6dba6d6dea59216923692dbfe2f229658fe0181812c56252339bee3"
    )
    assert record["decision_id"] == "RCV3-20260825-179"
    assert record["record_type"] == "DECISION"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    ...
    assert context["commit_context"] is False
    assert context["mission_id"] == "REAL_EXECUTION_BOOTSTRAP_CLOSURE_V1"
    assert context["phase"] == ("FIRST_REAL_C0_SINGLE_LEAGUE_CANARY_V1_PRECOMMIT_GOVERNANCE")
    assert context["worktree"] == "RobinFirstC0SingleLeague-20260825T120348Z"
    ...
    assert context["branch"] == "codex/first-real-c0-single-league-canary-v1"
    assert context["pr"] == "DRAFT_PENDING"
    assert context["writer"] == "C0"
    assert context["files"] == changed_files
    assert context["reused_evidence"] == list(
        predecessors := {
            "GOV.AUTHORIZATION.CHRONOS_LOOP53.028": expected_proof[0],
            "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.037": expected_proof[1],
            (
                "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST."
                "SOURCE_HASH.CONTRACT.V1.002"
            ): expected_proof[2],
            "TEMPORAL.FIRST_C0.SELECTOR.ROLLOVER.V1.001": expected_proof[3],
            "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.NETWORK.WINDOWS_API_TYPING.V1.001": (
                expected_proof[4]
            ),
            "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.003": expected_proof[5],
            "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.003": expected_proof[6],
            "TEMPORAL.OWNER_REVIEW_PACK.GENERATED_AT.CANONICALIZATION.V1.001": expected_proof[7],
            "GOV.FIRST_C0.PR69.CI.WORKTREE_SCOPE.ISOLATION.V1.001": expected_proof[8],
            "GOV.FIRST_C0.PRE_DNS.ORCHESTRATION.V1.001": expected_proof[9],
            (
                "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.008"
            ): expected_proof[10],
        }
    ...
    assert context["scope"] == {
        ...
        "tracked_files_maximum": 16,
        "outside_allowlist": [],
        "out_of_scope_dependencies": [],
        "convergence_manifest_location": "OUTSIDE_REPOSITORY",
        "convergence_manifest_sha256": context["scope"]["convergence_manifest_sha256"],
    }
    convergence_manifest_sha256 = context["scope"]["convergence_manifest_sha256"]
    assert len(convergence_manifest_sha256) == 64
    assert convergence_manifest_sha256 == convergence_manifest_sha256.lower()
    assert all(character in "0123456789abcdef" for character in convergence_manifest_sha256)
    assert context["owner_directive"] == {
        "utf8_bytes": 695,
        "sha256": "0cd4bfe98162cb5bd4ec611b69fdff3a53fdc7bdc899ed856f3d5d38dacb067b",
        "tracked_files_maximum": 16,
        "local_fix_loops": "unbounded_within_wall_clock",
        "ci_corrective_cycles_maximum": 3,
        "non_force_pushes_maximum": 4,
        "pull_requests": 1,
    ...
    assert context["mission_manifest"] == {
        ...
        "canonical_sha256": ("5f8d1db2586adbc397d1b4e814e85946c84327537872df6aeded0fbbbc5498c0"),
        "expires_at_utc": "2026-08-26T10:00:00Z",
    }
    assert context["contract_boundary"] == {
        "single_league_canary_authority_added": True,
        "five_league_authority_unchanged": True,
    ...
    assert context["runtime_boundary"] == {
        "primary_sport": "soccer_spain_la_liga",
        "fallback_sport": "soccer_germany_bundesliga",
        "official_physical_reads_maximum": 12,
        "preparation_cycles_maximum": 3,
        "provider_dns_resolutions_maximum": 1,
        "owner_review_pack_builds_maximum": 1,
        "owner_authorization_created": False,
        "future_single_real_request_only_under_david_continuation": True,
    }
    assert {item["name"] for item in context["targeted_tests"]} == {
        "first-c0-canary-selection",
        "live-canary-successor",
        "pre-dns-orchestration",
        "capture-suite",
        "council-pre-governance",
        "workflow-portability",
    }
    assert all(item["command"] and item["result"] == "PASS" for item in context["targeted_tests"])
    assert {item["name"] for item in context["static_validations"]} == {
        "ruff",
        "ruff-format",
        "strict-mypy",
        "bandit",
    ...
    assert all(
        item["command"] and item["result"] == "PASS" for item in context["static_validations"]
    )
    assert set(context["reviews"]) == {"DP6", "C4", "C2", "A2", "QA"}
    assert all(review["verdict"] == "ACCEPT" for review in context["reviews"].values())
    ...
    assert context["waivers"] == ["QA-P2-LOADER-DUPLICATION-001"]
    assert context["external_effects"] == {
        "official_schedule_reads": 0,
        "provider_dns_resolutions": 0,
    ...
    assert context["delivery_authority"] == {
        ...
        "non_force_pushes_maximum": 4,
        "pull_requests": 1,
        "planned_combined_commits": 1,
        "planned_initial_pushes": 1,
        "merge_authorized_now": False,
        "exact_head_ci_required": True,
        "postmerge_ci_required": True,
        "force_push_authorized": False,
        "provider_dns_during_engineering": False,
        "owner_review_pack_during_engineering": False,
    }
    assert context["outcome_boundary"] == {
        "c0_canary_only": True,
        ...
        "future_single_request_under_david_continuation": True,
    }
    for boundary in (
        "C0_CANARY_ONLY",
        "NO_LAB_READY",
        "NO_EDGE",
        "NO_PROMOTION",
        "FUTURE_SINGLE_REAL_REQUEST_ONLY_UNDER_DAVID_CONTINUATION",
    ):
        assert boundary in record["proposal"]
        assert boundary in record["objections"]
    ...
    assert len(base_graph["claims"]) == 440
    assert len(base_graph["decision_nodes"]) == 171
    assert len(base_graph["edges"]) == 738
    assert len(graph["claims"]) == 451
    assert len(graph["decision_nodes"]) == 172
    assert len(graph["edges"]) == 749
    ...
    for claim in base_graph["claims"]:
        if claim["claim_id"] in predecessors:
            claim = {
                **claim,
                "status": "SUPERSEDED",
                "superseded_by": predecessors[claim["claim_id"]],
            }
        expected_base_claims.append(claim)
    assert graph["claims"][:440] == expected_base_claims
    assert [claim["claim_id"] for claim in graph["claims"][440:451]] == expected_proof
    ...
    claim_artifacts = {
        ...
        expected_proof[5]: "tests/capture/test_live_canary_successor_v2.py",
        expected_proof[6]: "tests/capture/test_predns_orchestration_v1.py",
        expected_proof[7]: "tools/data-sourcing/build_owner_review_pack_v1.py",
        expected_proof[8]: "tests/council/test_robin_council_os_v3.py",
        expected_proof[9]: "src/robin/capture/predns_orchestration.py",
        expected_proof[10]: "reports/council/decision-ledger.jsonl",
    }
    for claim_id, artifact in claim_artifacts.items():
        assert claims[claim_id]["artifact"] == artifact
    ...
    assert graph["decision_nodes"][:171] == base_graph["decision_nodes"]
    assert graph["decision_nodes"][171] == {
        "decision_id": record["decision_id"],
        "ledger_record_hash": record["hash"],
    }
    assert graph["edges"][:738] == base_graph["edges"]
    assert graph["edges"][738:749] == [
        {
            "edge_id": f"EDGE.{number}",
            "from_claim_id": claim_id,
            "to_decision_id": record["decision_id"],
        ...
        for number, claim_id in zip(range(739, 750), expected_proof, strict=True)
    ]
...
def test_first_c0_authority_v4_recovery_is_exact_append_only_and_effect_free() -> None:
    base_revision = "5be50be64fe54af0b1a72089915b83119cf1e504"
    changed_files = [
        "configs/execution/real-execution-bootstrap-closure-v1.json",
        "src/robin/capture/bootstrap_contracts.py",
        "tests/capture/test_real_capture_workspace_bootstrap.py",
    ...
    expected_proof = [
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.031",
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.040",
        "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.V1.004",
        ("GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.SOURCE_HASH.CONTRACT.V1.005"),
        "TEMPORAL.FIRST_C0.SELECTOR.ROLLOVER.V1.004",
        (
            "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.WORKSPACE.FULL_CLONE."
            "JOB_ACCOUNTING.REGRESSION.V1.004"
        ),
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.006",
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.006",
        "GOV.FIRST_C0.PR69.CI.WORKTREE_SCOPE.ISOLATION.V1.004",
    ...
    if head_revision == base_revision:
        ...
        record_token = b'"decision_id":"RCV3-20260826-181"'
        ...
        assert len(ledger_bytes.splitlines()) == 174
        changed_scope = committed_changed_paths(ROOT, base_revision, closure_revision)
    assert changed_scope == set(changed_files)
    assert len(changed_scope) == 10
    ...
    assert len(base_ledger.splitlines()) == 173
    assert len(ledger_bytes.splitlines()) == 174
    assert ledger_bytes.startswith(base_ledger)
    assert ledger_bytes[len(base_ledger) :] == ledger_bytes.splitlines()[-1] + b"\n"
    assert b"\r" not in ledger_bytes
    ...
    records = [json.loads(line) for line in ledger_bytes.splitlines()]
    previous_record = records[172]
    record = records[173]
    assert previous_record["decision_id"] == "RCV3-20260825-180"
    assert previous_record["hash"] == (
        "c2b41fba7bb04542df8015abd8aed31f2976074a98a10f2c90cf3d2b7a65157f"
    )
    assert record["decision_id"] == "RCV3-20260826-181"
    assert record["record_type"] == "DECISION"
    assert record["decision"] == "PASS_AND_HOLD"
    assert record["dissent"] is None
    assert record["responsible"] == "C0"
    ...
    assert context["commit_context"] is False
    assert context["mission_id"] == "REAL_EXECUTION_BOOTSTRAP_CLOSURE_V1"
    assert context["phase"] == "FIRST_C0_AUTHORITY_V4_RECOVERY_V1_PREFLIGHT_HANDOFF"
    assert context["base_revision"] == base_revision
    assert context["candidate_parent_sha"] == base_revision
    assert context["head"] == base_revision
    assert context["branch"] == "codex/first-c0-authority-v4-recovery-v1"
    assert context["pr"] == "PENDING"
    assert context["writer"] == "C0"
    assert context["files"] == changed_files
    assert context["scope"] == {
        "tracked_file_count": 10,
        "tracked_files_maximum": 16,
        "primary_expected_paths": 10,
        "directly_consequential_extensions": [],
        "outside_closure": [],
    }
    assert context["owner_directive"] == {
        "utf8_bytes": 23954,
        "sha256": "3d3b43f68c0d339448e52de7ec66cce068646a4a006e267dfe063bffe2767f5e",
        "line_endings": "UTF8_LF_NO_BOM",
        "timestamp_utc": "2026-08-26T19:23:39.1089783Z",
        "locality": "LOCAL_FIXED_NTFS_OUTSIDE_REPOSITORY_AND_ONEDRIVE",
        "read_only_attribute": True,
    ...
    assert context["mission_manifest"] == {
        "source_hash": "3d3b43f68c0d339448e52de7ec66cce068646a4a006e267dfe063bffe2767f5e",
        "canonical_sha256": ("2e43edcb7aafd190161b81b9d251508e445a1dc85bcd7e4815e1a2f95fcf809f"),
        "artifact_sha256": ("23e93941093944a7c050467feae0c774d157958572e46ef03747a1c1875f6167"),
        "expires_at_utc": "2026-09-01T20:00:00Z",
        "mission_id": "REAL_EXECUTION_BOOTSTRAP_CLOSURE_V1",
        "authorized_stages": ["E1"],
        "maximum_stage": "E1",
    ...
    historical = context["historical_v3_authority"]
    assert historical == {
        "result": "OWNER_REVIEW_PACK_CREATED_BUT_EXPIRED_BEFORE_OWNER_AUTHORIZATION",
        "window_status": "MISSED_NOT_BACKDATED",
        "manifest_canonical_sha256": (
            "d895e0b2ddded2c9763d85a08efbd64dc0185d26f66bb2b73fbe52cc05411206"
        ),
        "pack_canonical_sha256": (
            "a2cdf5d2bc580fa40c86ac2e64d7aff6f1f97db24df27555c4295c8729696d2c"
        ),
        "pack_raw_sha256": "f14c2dec9ac578854b9b52773d0da027709cd2757e2b51a877c090aa4478ba48",
        "success_receipt_raw_sha256": (
            "908ba47b62657c0f815360e108cc2feb38fe7a6284021748130757ac02e9372b"
        ),
        "binding_canonical_sha256": (
            "6631af4625680716e0678f1872f35cc2053119d23c2bd9e5b8f3fd0cca7d23f8"
        ),
        "binding_raw_sha256": ("409e8a5c73f40ff40386999170d1329e2642dedb7691a2b0589d4a1494a76e93"),
        "marker_raw_sha256": "a1b7cbb95b5c7a221a7c50147e202d8d80649ec7049d0c132317803b0e51b28c",
        "resolution_claim_canonical_sha256": (
            "ed2185b98bf56c978261b30d3bc396099d1f2b698719f47dfff79f45eea664d7"
        ),
        "pack_currently_valid": False,
        "binding_currently_valid": False,
        "owner_authorized": False,
        "c0_executed": False,
        "paths_exist": True,
        "bytes_unchanged_at_precommit": True,
        "acl_unchanged_at_precommit": True,
        "reuse_authorized": False,
    }
    assert context["historical_marker_expectations"] == [
        {
            "authority_manifest_sha256": (
                "276e30fc4fe23600f654f93d9939b66b033d6002b68e86bcbfee0c34efe96c99"
            ),
            "raw_sha256": "22b0573e657b759cc848f8091aaa639f26a45f3c17fa43591983c4e9c59bc98e",
            "acl_sha256": "b032d14d479bbddd354c1a9a43250d4033f3029147a322f7177988fb4b15730a",
        },
        {
            "authority_manifest_sha256": (
                "d895e0b2ddded2c9763d85a08efbd64dc0185d26f66bb2b73fbe52cc05411206"
            ),
            "raw_sha256": "a1b7cbb95b5c7a221a7c50147e202d8d80649ec7049d0c132317803b0e51b28c",
            "global_acl_sha256": (
                "b032d14d479bbddd354c1a9a43250d4033f3029147a322f7177988fb4b15730a"
            ),
            "local_acl_sha256": (
                "f1cba44547f2210635f8b7e5d7114f9f1b2372bf970e2804260ac4d0c83945aa"
            ),
        },
    ]
    assert context["current_v4_authority"] == {
        "v3_accepted_as_current": False,
        "v4_global_marker_present": False,
        "v4_local_marker_present": False,
        "v4_binding_present": False,
        "v4_owner_review_pack_present": False,
        "v4_owner_authorization_present": False,
        "v4_c0_present": False,
        "global_marker_derivation": "MISSION_ID_PLUS_V4_CANONICAL_MANIFEST_SHA256",
    }
    assert context["delivery_authority"] == {
        "pull_requests": 1,
        ...
        "rebase_authorized": False,
        "squash_merge_authorized": False,
        "provider_dns_during_mission": False,
        "owner_review_pack_during_mission": False,
        "script_04_during_mission": False,
    }
    assert context["preflight_handoff_rule"] == {
        "script_02_autonomous": True,
        "script_03_autonomous": True,
        "script_04_automatic": False,
        "fresh_owner_presence_confirmation_required_after_preflight": True,
        "owner_presence_minimum_minutes": 20,
        "pack_creation_only": True,
        "c0_not_yet_authorized": True,
    }
    assert context["external_effects"] == {
        "official_schedule_reads": 0,
        "provider_dns": 0,
    ...
    assert context["outcome_boundary"] == {
        "authority_v4_engineered": True,
        "fresh_runtime_pending_postmerge": True,
        "launch_kit_v4_pending_postmerge": True,
        "owner_presence_handoff_pending_postmerge": True,
        "script_04_executed": False,
        "owner_pack_created": False,
        "owner_authorized": False,
        "c0_executed": False,
        "c0_audited": False,
    ...
    assert (len(graph["claims"]), len(graph["decision_nodes"]), len(graph["edges"])) == (
        471,
        174,
        769,
    )
    predecessors = {
        "GOV.AUTHORIZATION.CHRONOS_LOOP53.030": expected_proof[0],
        "GOV.EVIDENCE.REVISION_POLICY.CHRONOS_LOOP53.039": expected_proof[1],
        "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST.V1.003": expected_proof[2],
        (
            "GOV.AUTHORIZATION.REAL_EXECUTION_BOOTSTRAP.CLOSURE.MANIFEST."
            "SOURCE_HASH.CONTRACT.V1.004"
        ): expected_proof[3],
        "TEMPORAL.FIRST_C0.SELECTOR.ROLLOVER.V1.003": expected_proof[4],
        (
            "PORTABILITY.REAL_EXECUTION_BOOTSTRAP.WORKSPACE.FULL_CLONE."
            "JOB_ACCOUNTING.REGRESSION.V1.003"
        ): expected_proof[5],
        "GOV.OWNER_REVIEW_PACK.GENERATED_AT.CLI.REGRESSION.V1.005": expected_proof[6],
        "SECURITY.OWNER_REVIEW_PACK.DATETIME_FIX.ZERO_EFFECTS.V1.005": expected_proof[7],
        "GOV.FIRST_C0.PR69.CI.WORKTREE_SCOPE.ISOLATION.V1.003": expected_proof[8],
        (
            "GOV.COUNCIL.REAL_EXECUTION_BOOTSTRAP.CLOSURE.EVIDENCE.SUCCESSION.LEDGER.V1.010"
        ): expected_proof[9],
    }
    ...
    for claim in base_graph["claims"]:
        ...
        expected_base_claims.append(claim)
    assert graph["claims"][:461] == expected_base_claims
    assert [claim["claim_id"] for claim in graph["claims"][461:471]] == expected_proof
    claims = {claim["claim_id"]: claim for claim in graph["claims"]}
    for predecessor, successor in predecessors.items():
        assert claims[predecessor]["status"] == "SUPERSEDED"
        assert claims[predecessor]["superseded_by"] == successor
    ...
    for claim_id, artifact in claim_artifacts.items():
        ...
        assert claims[claim_id]["hash"] == closure_artifact_hash(artifact)
        assert claims[claim_id]["verified_by"] == ["C0", "C2", "C4", "DP6", "A2"]
    assert graph["decision_nodes"][:173] == base_graph["decision_nodes"]
    assert graph["decision_nodes"][173] == {
        "decision_id": record["decision_id"],
        "ledger_record_hash": record["hash"],
    }
    assert graph["edges"][:759] == base_graph["edges"]
    assert graph["edges"][759:769] == [
        {
            "edge_id": f"EDGE.{number}",
            "from_claim_id": claim_id,
            "to_decision_id": record["decision_id"],
        ...
        for number, claim_id in zip(range(760, 770), expected_proof, strict=True)
    ]
    assert [edge["edge_id"] for edge in graph["edges"]] == [
