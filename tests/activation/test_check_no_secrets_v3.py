...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "artifact.xml",
            '<dsn>postgresql://owner:not-a-real-password@'
            'ep-example.eu.neon.tech/robin?sslmode=require</dsn>',
            "Neon production DSN",
        ),
        (
            "temporary.txt",
...
def test_log_artifact_and_temporary_sentinels_are_detected(
    ...
    assert find_secret_literals([path]) == [f"{path.name}: {label}"]


def test_large_generated_text_is_scanned(tmp_path: Path) -> None:
    path = tmp_path / "large.log"
    path.write_text("x" * 2_100_000 + "\n" + "napi_" + "a" * 32, encoding="utf-8")
    assert find_secret_literals([path]) == [f"{path.name}: Neon API key"]
