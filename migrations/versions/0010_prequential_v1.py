...
Revision ID: 0010_prequential_v1
Revises: 0009_jalon12_observatory
"""
...
revision: str = "0010_prequential_v1"
down_revision: str | None = "0009_jalon12_observatory"
...
def _create_append_only_guards() -> None:
    ...
    if dialect == "postgresql":
        op.execute(
            """
            CREATE OR REPLACE FUNCTION robin_reject_prequential_mutation()
            RETURNS trigger
            LANGUAGE plpgsql
            AS $$
            BEGIN
                RAISE EXCEPTION 'append-only prequential table % cannot be mutated',
                    TG_TABLE_NAME;
            END;
            $$;
            """
        ...
        for table_name in CREATE_ORDER:
            op.execute(
                ...
                FOR EACH ROW
                EXECUTE FUNCTION robin_reject_prequential_mutation();
                """
            )
    elif dialect == "sqlite":
        for table_name in CREATE_ORDER:
            for operation in ("UPDATE", "DELETE"):
                op.execute(
                    ...
                    BEGIN
                        SELECT RAISE(
                            ABORT,
                            'append-only prequential table cannot be mutated'
                        );
                    END;
                    """
                )
    ...
    if dialect == "postgresql":
        for table_name in CREATE_ORDER:
            op.execute(
                ...
                f"trg_{table_name}_append_only ON {table_name}"
            )
        op.execute(
            "DROP FUNCTION IF EXISTS robin_reject_prequential_mutation()"
        )
    elif dialect == "sqlite":
        for table_name in CREATE_ORDER:
            for operation in ("update", "delete"):
