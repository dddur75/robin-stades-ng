...
PATTERNS = {
    ...
    "Neon production DSN": re.compile(
        ...
        r"ep-[a-z0-9-]+(?:\.[a-z0-9-]+)*\.neon\.tech/[^\s]+"
    ),
...
TEST_FIXTURE_MARKER = "SECRET_SCANNER_TEST_FIXTURE"
...
def find_secret_literals(paths: list[Path]) -> list[str]:
    ...
    for path in paths:
        ...
        fixture_markers_allowed = display_path.startswith("tests/")
        for label, pattern in PATTERNS.items():
            if any(
                pattern.search(line)
                and not (fixture_markers_allowed and TEST_FIXTURE_MARKER in line)
