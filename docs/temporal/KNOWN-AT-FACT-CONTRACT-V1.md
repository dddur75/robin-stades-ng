...
remplace pas l'heure de réception Robin. S'il est futur par rapport à
`known_at`, le fait devient `PROVIDER_TIMESTAMP_INCONSISTENT` et reste hors de
la vue stricte.
...
Les faits après cutoff sont conservés comme `LATE_FOR_CUTOFF` ; ceux connus à
ou après kickoff sont `POST_KICKOFF_ONLY` ; l'absence d'heure fiable est
...
Les snapshots de tags exigent exactement les 150 identifiants du registre. Un
tag TRUE/FALSE doit citer au moins un fait strict du même fixture et du même
cutoff ; un tag sans preuve demeure UNKNOWN. Le hash de manifeste porte sur
l'enveloppe canonique complète du fait, pas seulement sur sa valeur.
