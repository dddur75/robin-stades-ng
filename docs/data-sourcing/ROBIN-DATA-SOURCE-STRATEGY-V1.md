...
La priorité prospective unique est donc :

> **Commencer, après approbation séparée, une capture bornée des cotes courantes The Odds API (`h2h` et `totals`) sur cinq ligues, avec six fenêtres pour la Ligue 1 et trois pour les quatre autres ligues, et un reçu LOOP55-compatible pour chaque réponse.**
...
La politique proposée reprend les sport keys et fenêtres déjà documentés localement, sans activer le pipeline :

- Ligue 1 : `J-7`, `J-3`, `J-1`, `H-6`, `H-2`, `NEAR_KICKOFF` ;
- Premier League, Liga, Bundesliga, Serie A : `J-1`, `H-2`, `NEAR_KICKOFF` ;
- `NEAR_KICKOFF` cible T-15 minutes et n'accepte que l'intervalle `[T-20m, T-10m]` ; après kickoff, la capture est mise en quarantaine et ne remplace jamais la fenêtre ;
- un appel par compétition et fenêtre, `regions=eu`, `markets=h2h,totals`, format décimal/ISO ;
- premier essai : un seul appel pour la prochaine journée Ligue 1 à la première fenêtre encore future ; jamais de backdate ;
- scope initial : 1X2 et O/U 2,5 ; garder la réponse complète mais retarder DNB, double chance, BTTS et AH.

Projection centrale locale existante : 1 752 fixtures, 648 appels compétition-fenêtre, 1 296 crédits/saison, soit 24,92 crédits/semaine en moyenne. À deux crédits par appel, le plan Starter affiché à 0 USD et 500 crédits/mois suffit à l'hypothèse centrale ; le prix et les CGU doivent être revérifiés avant activation. Le nombre de bookmakers, de quotes et les octets restent volontairement `null` jusqu'au premier snapshot réel.

Le pilote ne passe que si :

- les bytes, le SHA-256, le schéma et les douze champs de reçu sont complets ;
- au moins un événement se mappe sans ambiguïté à une fixture vérifiée ;
- les triplets 1X2 et les deux côtés O/U portent une structure complète ;
- au moins cinq bookmakers donnent un 1X2 complet pour 80 % des fixtures mappées ;
- tous les temps sont UTC, toute dérive future est mise en quarantaine ;
- un replay sans réseau reproduit hash, schéma et projections ;
- aucun secret ni payload brut n'entre dans Git ;
- le coût réel, les octets, marchés et bookmakers sont mesurés.

Le passage à cinq ligues exige deux journées, au moins cinq fixtures, deux fenêtres critiques, 100 % de reçus valides, des droits de rétention confirmés et aucune violation des locks production. Le détail exécutable comme décision, pas comme activation, est dans la [recommandation](../../reports/data-sourcing/first-prospective-capture-recommendation-v1.json).
...
6. **Coût.** Les 1 296 crédits/an sont une projection, non une promesse ; le quota doit être contrôlé par réponse et par pic mensuel.
